...
import {get, getcheckone, getcheckstatus} from "../../services/contract.service";
...
const ListNVLDCheck = (props) => {
  ...
  const getType = async ()=>{
    ...
    const contract = await getcheckone();
  ...
  const setnewcontract = async()=>{
    dispatch({type:"SHOW_LOADING"});
    const co = await getcheckstatus(status);
    setNewcontract(co);
    dispatch({type:"HIDE_LOADING"});
  ...
  return (
    ...
    <Box m="20px">
      ...
      <div className="container shadow" style={{display:'grid'}}>
                <h1 style={{margin:'auto', marginTop:'24px'}}>NVLD CHECK CONTRACTS</h1>
                  ...
                  <div class="input-group mb-3 col-md-6">
                    ...
                    <select class="custom-select" id="inputGroupSelect03" onClick={handleStatus}>
                      ...
                      <option value="0">Hợp đồng mới tạo đã có báo cáo lắp đặt chờ duyệt</option>
                      {/* <option value="1">Hợp đồng mới tạo có báo cáo lắp đặt được duyệt</option> */}
                      <option value="2">Hợp đồng mới tạo có báo cáo lắp đặt không được duyệt</option>
                            ...
                            newcontract.map((e,k)=>{
                                return (
                                    <tr key={k}>
                                        ...
                                        <td style={{}}>
                                          {(e.checkStatus==null)?<Link to={"/contract-edit/"+e.contractId}>
                                          ...
                                          {(e.checkStatus==0)?<Link >
                                            <button  className="btn btn-outline-warning">
                                            CHỜ DUYỆT
                                            </button> 
                                            </Link>:<></>}
                                            {(e.checkStatus==1)?<Link>
                                            <button  className="btn btn-outline-success">
                                            SUCCESS
                                            </button> 
                                            </Link>:<></>}
                                            {(e.checkStatus==2)?<Link to={"/contract-edit/"+e.contractId}>
                                            <button  className="btn btn-outline-success">
                                            AGAIN
                                            </button> 
                                            </Link>:<></>}
